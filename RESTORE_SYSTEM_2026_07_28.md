...
**Date of Backup**: July 28, 2026  
**System Target**: ESP32-S3 Sanitary Vending Machine Controller  
**Hardware Board**: ESP32-S3-DevKitC-1 (Serial / Upload Port: `COM7`)  
...
3. [File 1: platformio.ini (Build & Dependency Config)](#file-1-platformioini)
4. [File 2: src/html_page.h (Web Interface Header)](#file-2-srchtml_pageh)
5. [File 3: src/main.cpp (Master ESP32-S3 Firmware)](#file-3-srcmaincpp)
...
| **GPIO 3** | EC11 Rotary Encoder | ENC_SW | Knob Push Button |
| **GPIO 5** | Piezo Buzzer Positive (+) | BUZZER | Audio Chime Tone |
| **GPIO 6** | DRV8833 Motor Driver | IN4 | Stepper Coil B- |
| **GPIO 7** | DRV8833 Motor Driver | IN3 | Stepper Coil B+ |
| **GPIO 8** | EC11 Rotary Encoder | ENC_B | Quadrature Phase B |
| **GPIO 9** | MFRC522 RFID Reader | SDA / CS | RFID Chip Select |
| **GPIO 10** | DRV8833 Motor Driver | IN1 | Stepper Coil A+ |
| **GPIO 11** | DRV8833 Motor Driver | IN2 | Stepper Coil A- |
| **GPIO 12** | **SHARED**: ST7789 TFT & RC522 RFID | SCLK / SCK | Shared SPI Clock |
| **GPIO 13** | **SHARED**: ST7789 TFT & RC522 RFID | MOSI / SDA | Shared SPI Master Out |
| **GPIO 14** | ST7789 TFT Display | RST | Display Reset |
| **GPIO 15** | ST7789 TFT Display | DC | Display Data / Command |
| **GPIO 16** | ST7789 TFT Display | CS | Display Chip Select |
| **GPIO 17** | ST7789 TFT Display | BLK | Display Backlight PWM |
| **GPIO 18** | EC11 Rotary Encoder | ENC_A | Quadrature Phase A |
| **GPIO 46** | Push Button (KEY0) to GND | KEY0 | Navigation Back |
...
## File 2: src/html_page.h

Save the code below as `src/html_page.h`:
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function connectMqtt() {
        ...
        try {
            ...
            logConsole(`MQTT Init Error: ${e.message}.`, 'error');
        }
    }
    ...
    function onConnectSuccess() {
        ...
        logConsole(`Cloud MQTT Connected! Subscribed: ${statusTopic}`, 'success');
    ...
    function onConnectFailure(response) {
        ...
        logConsole(`Cloud MQTT Connection Failed. Retrying...`, 'error');
    ...
    function onConnectionLost(responseObject) {
        isConnectedMqtt = false;
        logConsole(`Cloud MQTT Connection Lost`, 'error');
    ...
    function sendMqttCommand(cmdStr) {
        if (!isConnectedMqtt || !mqttClient) {
            logConsole('MQTT not connected yet.', 'error');
            return;
        }
    ...
    btnConnectIp.addEventListener('click', () => {
        ...
        ipModeBadge.textContent = `MACHINE #${machineId}`;
        logConsole(`Switched Cloud Target to Machine #${machineId}`, 'info');
...
To view or restore the full 2000+ line C++ firmware, refer to **[src/main.cpp](file:///d:/Developments/screen_testing_v1/src/main.cpp)** in your active workspace repository!
